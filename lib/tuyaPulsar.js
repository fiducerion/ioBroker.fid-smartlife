...
class TuyaPulsar extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.cloudRef       = opts.cloudRef || null;    // TuyaCloud instance fuer UID-Lookup
  ...
  async _connectLoop() {
    ...
    // Alten Client komplett aufraeumen bevor neuer kommt
    if (this.mqttClient) {
      try {
        const oldClient = this.mqttClient;
        this.mqttClient = null;
    ...
    try {
      ...
      // Diagnose: alle source-Topics + Felder loggen
      const cfgKeys = Object.keys(this.mqttCfg).join(',');
      const topicsObj = this.mqttCfg.source_topic;
      const topicsStr = (topicsObj && typeof topicsObj === 'object')
        ? JSON.stringify(topicsObj)
        : String(topicsObj || '');
      this.logger('info', 'TuyaPulsar: Access-Config ok, broker=' + this._brokerHost()
        + ', client_id=' + (this.mqttCfg.client_id || '?').substring(0, 16) + '...'
        + ', source_topics=' + topicsStr
        + ', expire=' + this.mqttCfg.expire_time
        + 's, cfgKeys=[' + cfgKeys + ']');
  ...
  // ---------- Auth: /v1.0/open-hub/access/config ----------
  // (Smart-Home Endpoint - NICHT /v1.0/iot-03/open-hub/access-config, das ist
  // fuer IoT Industry Projects und macht 1106 permission deny.
  // Pfad bestaetigt aus HA tuya-iot-py-sdk source und Tuya WebRTC-Doku.)

  async _getAccessConfig() {
    // Wir brauchen ein Cloud-Token + UID. Wenn der user uns einen liefert
    // via getCloudToken, super. Sonst holen wir selber einen.
    let accessToken;
    ...
    // UID kommt im Token-Response - TuyaCloud-Instance setzt this.uid wenn wir
    // den getCloudToken-Callback nutzen (siehe lib/tuyaCloud.js ensureToken).
    if (!this.uid && this.cloudRef && this.cloudRef.uid) {
      this.uid = this.cloudRef.uid;
    }
    if (!this.uid) {
    ...
    const path = '/v1.0/open-hub/access/config';   // SMART HOME endpoint!
    const body = {
      uid:      this.uid,
      link_id:  linkId,
      link_type: 'mqtt',
      topics:   'device',
      msg_encrypted_version: '2.0'  // 2.0 = AES-GCM (in Tuya Platform aktiviert),
                                    // 1.0 = ECB (alt). Wenn dein Tuya-Project auf
                                    // GCM steht, MUSS hier 2.0. Falls 1.0 wird das
                                    // automatisch via 'pv'-Feld im outer payload erkannt.
    };
  ...
  async _mqttConnect() {
    return new Promise((resolve, reject) => {
      ...
      this.mqttClient.on('close', () => {
        ...
        // Reconnect nur wenn KEIN anderer Reconnect bereits laeuft
        if (!this.stopped && !this._reconnectScheduled) {
  ...
  _onMqttMessage(topic, payload) {
    // Raw message event - fuer Diagnose damit man sieht ob ueberhaupt was vom
    // Tuya-Broker kommt (unabhaengig davon ob Decrypt/Parse klappt).
    ...
    try {
      ...
      this.logger('warn', 'TuyaPulsar: msg payload not JSON (raw=' + payload.length + ' bytes)');
      return;
    }
    if (!outer.data) {
