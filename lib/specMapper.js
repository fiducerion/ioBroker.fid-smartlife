...
let _schemaDb = null;       // 31 MB ungezipt - lazy laden
let _schemaDbLoaded = false;

function _loadSchemaDb(logger) {
  if (_schemaDbLoaded) return _schemaDb;
  _schemaDbLoaded = true;
  // Erst .gz versuchen (npm-package shipping format), dann unkomprimiert (dev-clone)
  const gzPath   = path.join(__dirname, 'schema.json.gz');
  const rawPath  = path.join(__dirname, 'schema.json');
  const tStart = Date.now();
  try {
    if (fs.existsSync(gzPath)) {
      const gz = fs.readFileSync(gzPath);
      const json = zlib.gunzipSync(gz).toString('utf-8');
      _schemaDb = JSON.parse(json);
      if (logger) logger('info', 'Schema-DB geladen (gzip): ' + Object.keys(_schemaDb).length + ' Eintraege in ' + (Date.now() - tStart) + 'ms');
    } else if (fs.existsSync(rawPath)) {
      _schemaDb = JSON.parse(fs.readFileSync(rawPath, 'utf-8'));
      if (logger) logger('info', 'Schema-DB geladen (raw): ' + Object.keys(_schemaDb).length + ' Eintraege in ' + (Date.now() - tStart) + 'ms');
    } else {
      throw new Error('weder schema.json.gz noch schema.json gefunden');
    }
  } catch (e) {
    _schemaDb = null;
...
function extractMeta(dpDef) {
  ...
  return {
    ...
    scale:  scaleVal,
    enums:  enums,
    friendly: (dpDef && (dpDef.codeReal || dpDef.code || dpDef.canon)) || 'dp',
    chineseName: (dpDef && dpDef.name) || undefined,
