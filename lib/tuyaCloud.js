...
class TuyaCloud {
  ...
  request(method, path, query, body, useToken) {
    return new Promise((resolve, reject) => {
      ...
      // v0.10.8: KRITISCH gegen ECONNRESET-Crash bei DSL-Ausfall.
      // Wenn die Internet-Verbindung abbricht waehrend ein Request laeuft,
      // wirft der UNTERLIEGENDE TLS-Socket ein 'error'-Event (TLSWrap.
      // onStreamRead -> read ECONNRESET). Dieses Event ist NICHT dasselbe wie
      // req.on('error') - es kommt auf dem Socket-Objekt eine Ebene tiefer und
      // KANN nach done()/req.destroy() noch kommen (Late-Error). Ohne Listener
      // dort -> uncaughtException -> Adapter terminated (code 6).
      // Wir haengen uns deshalb an den Socket sobald er zugewiesen wird.
      req.on('socket', (socket) => {
        if (socket && typeof socket.on === 'function') {
          socket.on('error', (e) => {
            // Late-Socket-Error abfangen. done() ist idempotent (settled-Guard).
            done(new Error('socket error: ' + (e && e.message || String(e))));
          });
        }
      });
  ...
  async ensureToken() {
    ...
    const uid         = res && res.result && res.result.uid;
  ...
  /** Detail-Info eines Geraets incl. ip (falls online) */
  async getDeviceInfo(deviceId) {
    await this.ensureToken();
    const res = await this.request('GET', '/v1.0/devices/' + deviceId, null, null, true);
