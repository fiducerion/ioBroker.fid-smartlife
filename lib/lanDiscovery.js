...
function tryDecryptUdp(buf) {
  // Tuya v3.3 Broadcasts haben 16 Byte Header + payload + 8 Byte Suffix.
  // (Bei TCP-Control-Paketen ist nach Header noch '3.3' + 12 bytes padding,
  // aber bei UDP-Broadcasts NICHT.)
  // Wir probieren beide Offsets - skip16 ist der Broadcast-Standard.
  if (buf.length < 24) return null;
  const tries = [16, 20];
  for (const skip of tries) {
    try {
      const body = buf.subarray(skip, buf.length - 8);
      ...
      // Wenn's JSON-aehnlich aussieht, gut. Sonst weiter probieren.
