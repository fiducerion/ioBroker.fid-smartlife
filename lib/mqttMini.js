...
class MqttMiniClient extends EventEmitter {
  ...
  _handlePacket(type, firstByte, body) {
    switch (type) {
      case TYPE_CONNACK: {
        ...
        if (returnCode === 0) {
          this.connected = true;
          this.logger('debug', 'MqttMini: CONNACK ok');
      ...
      case TYPE_SUBACK: {
        ...
        this.logger('debug', 'MqttMini: SUBACK pid=' + pid + ' rc=' + returnCode);
        if (returnCode >= 0x80) {
  ...
  subscribe(topic, qos) {
    ...
    try {
      this.socket.write(Buffer.concat([fixedHeader, remaining]));
      this.logger('debug', 'MqttMini: SUBSCRIBE sent for ' + topic + ' (qos=' + qos + ', pid=' + pid + ')');
      return true;
    } catch (e) {
