...
class MqttMiniClient extends EventEmitter {
  constructor(opts) {
    ...
    // v0.10.8: KRITISCH gegen Crash bei DSL-Ausfall.
    // EventEmitter.emit('error') THROWED synchron eine Exception wenn KEIN
    // 'error'-Listener registriert ist. Wenn die Internet-Verbindung abbricht
    // (ECONNRESET) und der Consumer gerade keinen Listener hat (z.B. waehrend
    // Reconnect oder nach cleanup), crasht das den ganzen Adapter (code 6).
    // Ein permanenter no-op Listener verhindert das - der echte Consumer kann
    // zusaetzlich seinen eigenen on('error') registrieren.
    this.on('error', () => { /* swallow - prevents uncaughtException */ });
  }
  ...
  connect() {
    ...
    this.socket.on('error', (err) => {
      ...
      // v0.10.8: Socket explizit zerstoeren damit keine weiteren Late-Read-
      // Errors (TLSWrap.onStreamRead ECONNRESET) auf diesem Socket kommen.
